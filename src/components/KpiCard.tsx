...
const NEGATIVE_METRIC_IDS = new Set(['noShows', 'cancelledCalls', 'reschedules']);
...
export default function KpiCard({ id, label, value, format, trend, accentColor, sparkline }: KpiCardData) {
  ...
  const isNegMetric = NEGATIVE_METRIC_IDS.has(id);
  const trendGood = isNegMetric ? trend < 0 : trend >= 0;
  const gradId = `sg-${id}`;

  return (
    <div
      className="relative flex flex-col rounded-xl border overflow-hidden transition-all duration-300 hover:scale-[1.02] hover:shadow-lg"
      style={{
